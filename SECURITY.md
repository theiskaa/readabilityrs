...
- **Deeply nested markup is bounded, but expensive.** The recursive tree walkers now stop descending past 256 levels and drop the subtree, so a nested document no longer aborts the process. Cost is the remaining problem: several cleanup passes scale worse than linearly with element count, so a document with tens of thousands of elements takes seconds to minutes regardless of nesting. Bound input size and apply your own timeout.
